...
(function (global, factory) {
...
}(this, function (exports,d3$1,_) { 'use strict';
    ...
    function tooltip(tooltipVariables) {
      var tip = d3Tip().parent(document.getElementById('chart')).offset([0, 0]).direction('n').attr('class', 'd3-tip').html(function (d) {
        ...
        tooltipVariables.forEach(function (e) {
          ...
          if (typeof e.format !== 'undefined') {
            if (e.type === 'time') {
              ...
              var currentFormat = d3$1.timeFormat(e.format);
              currentValue = currentFormat(inputValue);
            } else {
              // number formatting
              var _inputValue = Number(f[e.name]);
              var _currentFormat = d3$1.format(e.format);
    ...
    function showTooltip(d, i, options) {
    ...
    function removeTooltip(d, i, options) {
      var idVariable = options.idVariable;
      var xVariable = options.xVariable;
      var yVariable = options.yVariable;
      var tip = options.tip;
    ...
    function drawVoronoiScatterplot(selector, inputData, options) {
      ...
      var div = d3$1.select(selector).append('div').attr('id', 'chart');
      ...
      var color = d3$1.scaleOrdinal().range(['#1f78b4', '#ff7f00', '#33a02c', '#e31a1c', '#6a3d9a', '#b15928', '#a6cee3', '#fdbf6f', '#b2df8a', '#fb9a99', '#cab2d6', '#ffff99']);
      ...
      var xScale = d3$1.scaleLinear().range([0, width]);
      ...
      switch (yScaleType) {
        case 'power':
          yScale = d3$1.scalePow().range([height, 0]).exponent(yScaleExponent);
          break;
        default:
          yScale = d3$1.scaleLinear().range([height, 0]);
      }
      ...
      if (typeof globalExtents !== 'undefined') {
        ...
        xScale.domain(d3$1.extent(data, function (d) {
          return d[xVariable];
        }));
        // .nice();
        yScale.domain(d3$1.extent(data, function (d) {
          return d[yVariable];
        })).nice();
      }
      ...
      // Set new x-axis
      var xAxis = d3$1.axisBottom().ticks(4).tickSizeOuter(0)
      ...
      var xAxisYTranslate = d3$1.max([0, yScale.domain()[0]]);

      // Append the x-axis
      wrapper.append('g').attr('class', 'x axis').attr('transform', 'translate(' + 0 + ', ' + yScale(xAxisYTranslate) + ')').call(xAxis);

      var yAxis = d3$1.axisLeft().ticks(6) // Set rough # of ticks
      .scale(yScale);
      ...
      if (typeof rVariable !== 'undefined') {
        var _rScale = d3$1.scaleSqrt().range([mobileScreen ? 1 : 2, mobileScreen ? 10 : 16]).domain(d3$1.extent(data, function (d) {
          return d[rVariable];
        }));
      }
      ...
      function update(data, options) {
        ...
        svg.on('mousemove', function () {
          if (!svg._voronoi) {
            ...
            svg._voronoi = d3$1.voronoi().x(function (d) {
              return xScale(d[xVariable]);
            }).y(function (d) {
              return yScale(d[yVariable]);
            })(data);
      ...
      function click() {
        if (axisVisible) {
          d3$1.selectAll('.y.axis').style('opacity', 0);
          d3$1.selectAll('.x.axis text').style('opacity', 0);
          d3$1.selectAll('.x.axis .tick').style('opacity', 0);
          axisVisible = false;
        } else {
          d3$1.selectAll('.axis').style('opacity', 1);
          d3$1.selectAll('.x.axis text').style('opacity', 1);
          d3$1.selectAll('.x.axis .tick').style('opacity', 1);
          axisVisible = true;
        }
      }

      d3$1.selectAll('.chartWrapper').on('click', function () {
        click();
      });
